...
def get_handler_dataset_attr(data_args, raw_datasets):
    ...
    if "AlpacaStyle" in data_args.handler_name:
        ...
        column_names = ["prompt", "query", "response", "history", "system", "chosen", "rejected", "reward"]
        ...
        column_names = ["messages", "tags", "system", "tools", "chosen", "rejected", "kto_tag", "reward"]
...
def convert_alpaca_to_intermediate(sample: Dict[str, List[Any]], dataset_attr: "InstructionDatasetAttr"):
    ...
    """
    outputs = {"prompt": [], "response": [], "system": [], "tools": [], "reward": []}
    prompt = []
    ...
    outputs["reward"].append(sample[dataset_attr.reward] if dataset_attr.reward else 0.0)
