...
class OPGTrainer(BaseTrainer):
    ...
    @staticmethod
    def loss_func(input_tensor: torch.Tensor, reward: torch.Tensor, output_tensor: torch.Tensor):
        """Loss function.
        ...
        metrics = {}        
        metrics['reward'] = prob.detach().mean() * reward[0]
        metrics['pg loss'] = averaged_loss[0]

        return loss, metrics
    
        # loss_mask = input_tensor

        # output_tensor *= reward[0]
        # losses = output_tensor.float()
        # loss_mask = loss_mask[..., 1:].view(-1).float()
